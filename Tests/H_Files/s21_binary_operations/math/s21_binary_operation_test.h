...
#define BINARY_ARITHMETIC_TEST(func, operation_type)                  \
  START_TEST(func##_test) {                                           \
    ...
    s21_decimal sharp_result;                                         \
                                                                      \
    int success_sharp = get_result_operation_binary(                  \
        &number_1, &number_2, &sharp_result, operation_type);         \
                                                                      \
    int success_lib = func(number_1, number_2, &lib_result);          \
                                                                      \
    TEST_OUTPUT;                                                      \
                                                                      \
    ck_assert_int_eq(success_lib, success_sharp);                     \
    if (success_sharp == 0) assert_decimal(lib_result, sharp_result); \
    CloseGenerators();                                                \
  }                                                                   \
  END_TEST
